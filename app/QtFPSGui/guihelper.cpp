...
#ifdef MODE_WINDOW
void GuiHelper::setup(Ui::MainWindowDialog *mw) {
    canvas = mw->canvas;
...
void GuiHelper::disableOnConnected() {
#ifdef MODE_WINDOW
...
void GuiHelper::enableOnDisconnected() {
#ifdef MODE_WINDOW
...
void GuiHelper::drawImage(void *data, ImageType type) {
...
void GuiHelper::saveLastImage(const QString &_filename) {
...
void GuiHelper::setResult(const QString &line1, const QString &line2) {
    QString r = line1 + "\n" + line2;
#ifdef OUTPUT_DEBUG
    qDebug() << "setResult:" << r;
#else
...
bool GuiHelper::confirm(const QString &message, const QString &title) {
#ifdef MODE_WINDOW
...
QString GuiHelper::getOpenFilename(const QString &exts) {
#ifdef MODE_WINDOW
...
QString GuiHelper::getSaveFilename(const QString &exts, const QString &suggested) {
#ifdef MODE_WINDOW
...
qint64 GuiHelper::writeAll(const QString &filename, uchar *buffer, uint size) {
    QFile file(filename);
...
qint64 GuiHelper::readAll(const QString &filename, uchar *buffer, uint size) {
    QFile file(filename);
...
QString GuiHelper::formatSerialNumber(uchar *p) {
    QString sn = "";
    for(int i = 0; i < 16; i++) {
